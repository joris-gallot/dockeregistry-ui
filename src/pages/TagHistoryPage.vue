...
const platforms = ref<
  Array<{ os: string; architecture: string; variant?: string; digest: string }>
>([]);
  ...
  <div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
      <Button
        variant="ghost"
        size="icon"
        @click="router.push({ name: 'taglist', params: { image } })"
      >
      ...
      <div class="flex gap-2">
        <Dialog>
          <DialogTrigger as-child>
            <Button variant="outline" size="sm" :disabled="!dockerfile"
              >Dockerfile</Button
            >
          </DialogTrigger>
    ...
    <Tabs
      ...
      @update:model-value="switchPlatform($event as string)"
    >
      <TabsList>
        <TabsTrigger v-for="p in platforms" :key="p.digest" :value="p.digest">
    ...
    <div v-else class="space-y-3">
      <Card
        ...
        :key="idx"
        :class="{ 'opacity-60': layer.emptyLayer }"
        class="p-0"
      >
          ...
          <div class="flex items-start justify-between gap-4">
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 mb-1">
                <Badge
                  :variant="layer.emptyLayer ? 'outline' : 'secondary'"
